...
namespace Owasp.Esapi
{
    ...
    public class Randomizer : IRandomizer
    {
        ...
        /// <inheritdoc cref="Owasp.Esapi.Interfaces.IRandomizer.GetRandomLong()" />
        public long GetRandomLong()
        {
            Random random = new Random();
            return ((long)random.Next(32) << 32) + random.Next(32);
        }
        /// <inheritdoc cref="Owasp.Esapi.Interfaces.IRandomizer.GetRandomReal(float min, float max)" />
        public float GetRandomReal(float min, float max)
        {
            Random random = new Random();
            float factor = max - min;
            float nextFloat = random.Next(24) / ((float)(1 << 24));
            return nextFloat * factor + min;
        }
